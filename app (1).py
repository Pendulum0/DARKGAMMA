...
# greeks that are INTRINSICALLY signed (call +, put −): don't apply the dealer
# call+/put− flip on top, or puts get double-signed and the net can never go negative.
NATURAL_SIGN = {"delta", "rho"}
...
def exposure(df, greek_key, spot, basis="oi"):
    """Signed dollar exposure per row.
    Symmetric greeks (gamma, vega, charm, ...): dealer convention, call +, put −.
    Intrinsically-signed greeks (delta, rho): use their own sign — no extra flip,
    so net delta can correctly go negative when puts dominate (matches standard DEX)."""
    qty = df[basis].values
    if greek_key in NATURAL_SIGN:
        sign = 1.0
    else:
